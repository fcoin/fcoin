...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #FCoinTEST2\r");
            Send(hSocket, "WHO #FCoinTEST2\r");
        } else {
            // randomly join #FCoin00-#FCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #FCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #FCoin%02d\r", channel_number).c_str());
        }

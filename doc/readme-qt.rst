...
Alternatively, install Qt Creator and open the `FCoin-qt.pro` file.

An executable named `FCoin-qt` will be built.
...
A warning for people using the *static binary* version of FCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of FCoin is linked against libdb5.3.
...
installed causes FCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
